...
int is_square_attacked(int square, int side) {
  ...
  if (get_archbishop_attacks(square, board.occupancies[both]) & ((side == white) ? board.bitboards[A] : board.bitboards[a]))
    return 1;

  if (get_rook_attacks(square, board.occupancies[both]) & ((side == white) ? board.bitboards[R] : board.bitboards[r]))
    return 1;

  if (get_chancellor_attacks(square, board.occupancies[both]) & ((side == white) ? board.bitboards[C] : board.bitboards[c]))
    return 1;

  if (get_queen_attacks(square, board.occupancies[both]) & ((side == white) ? board.bitboards[Q] : board.bitboards[q]))
    return 1;

  if (get_amazon_attacks(square, board.occupancies[both]) & ((side == white) ? board.bitboards[M] : board.bitboards[m]))
    return 1;
